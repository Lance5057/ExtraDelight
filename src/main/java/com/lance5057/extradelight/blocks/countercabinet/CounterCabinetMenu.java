		...
		if (tileEntity != null) {
			...
			for (int row = 0; row < 3; ++row) {
				for (int column = 0; column < 9; ++column) {
					this.addSlot(new HideableSlot(tileEntity.getItemHandler(), (row * 9) + column,
...
//			}
	...
	private static CounterCabinetBlockEntity getTileEntity(final Inventory playerInventory,
			final FriendlyByteBuf data) {
		Objects.requireNonNull(playerInventory, "playerInventory cannot be null");
		Objects.requireNonNull(data, "data cannot be null");
		final BlockEntity tileAtPos = playerInventory.player.level().getBlockEntity(data.readBlockPos());
		if (tileAtPos instanceof CounterCabinetBlockEntity) {
			return (CounterCabinetBlockEntity) tileAtPos;
		}
		throw new IllegalStateException("Tile entity is not correct! " + tileAtPos);
	}

	public CounterCabinetMenu(final int windowId, final Inventory playerInventory, final FriendlyByteBuf data) {
	...
	public boolean stillValid(Player pPlayer) {
		return canInteractWithCallable.evaluate((p_38916_, p_38917_) -> {
			return !(p_38916_.getBlockState(p_38917_).getBlock() instanceof CounterCabinetBlock) ? false
