...
public class WildCornFeature extends Feature<WildCornConfig> {

	public WildCornFeature() {
		super(WildCornConfig.CODEC);
...
//					Blocks.DIRT, Blocks.COARSE_DIRT, Blocks.RED_SAND, Blocks.SAND))));
	...
	@Override
	public boolean place(FeaturePlaceContext<WildCornConfig> pContext) {
		WildCornConfig WildCornConfig = pContext.config();
	...
	private void spawn(LevelAccessor worldIn, RandomSource rand, BlockPos generatingPos) {
		worldIn.setBlock(generatingPos.above(),
				ExtraDelightBlocks.CORN_TOP.get().defaultBlockState().setValue(CornTop.AGE, 3), 4);
		worldIn.setBlock(generatingPos,
				ExtraDelightBlocks.CORN_BOTTOM.get().defaultBlockState().setValue(CornBottom.AGE, 3), 4);
	}
