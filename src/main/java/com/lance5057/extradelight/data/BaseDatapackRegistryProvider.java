...
public abstract class BaseDatapackRegistryProvider extends DatapackBuiltinEntriesProvider {
	...
	protected BaseDatapackRegistryProvider(PackOutput output, CompletableFuture<HolderLookup.Provider> registries,
			RegistrySetBuilder registrySetBuilder, String modid) {
	...
	protected static void registerPlacedFeature(BootstrapContext<PlacedFeature> context, ResourceLocation name,
	...
	protected static void registerPlacedFeature(BootstrapContext<PlacedFeature> context, ResourceLocation name,
			ResourceLocation retrogenName, Boolean2ObjectFunction<List<PlacementModifier>> placementModifiers) {
		...
		Reference<ConfiguredFeature<?, ?>> retrogenConfiguredFeature = configuredFeatures
				.getOrThrow(configuredFeature(retrogenName));
		context.register(placedFeature(name.withSuffix("_retrogen")),
				new PlacedFeature(retrogenConfiguredFeature, placementModifiers.get(true)));
	}
